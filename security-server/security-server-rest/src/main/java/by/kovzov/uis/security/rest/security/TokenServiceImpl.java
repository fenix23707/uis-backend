...
public class TokenServiceImpl implements TokenService {
    ...
    public JwtAuthenticationDto createTokens(Authentication authentication) {
        if (!(authentication.getPrincipal() instanceof UserSecurity userSecurity)) {
            throw new BadCredentialsException(
                MessageFormat.format("Principal {0} is not of Userauth type", authentication.getPrincipal().getClass()));
        }
    ...
    private String createAccessToken(Authentication authentication) {
        UserSecurity userScurity = (UserSecurity) authentication.getPrincipal();
        Instant now = Instant.now();
        JwtClaimsSet claimsSet = JwtClaimsSet.builder()
            .issuer("uis")
            .issuedAt(now)
            .expiresAt(now.plus(10, ChronoUnit.MINUTES))
            .subject(userScurity.getUsername())
            .build();
